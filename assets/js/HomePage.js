...
async function fetchDashboard() {
  ...
  try {
    ...
    renderActivity(json.data.recent_borrowed);
    const data = {
      currently_borrowing: json.data.currently_borrowing,
      due_soon: json.data.due_soon,
      over_due: json.data.over_due,
      total_borrowed: json.data.total_borrowed,
    };
    renderMetrics(data);
...
function bookStatus(status, dueDate) {
  ...
  if (status === "OVERDUE")
    return { label: "Overdue", cls: "badge-overdue", type: "overdue" };
  if (status === "PENDING")
    return { label: "Pending", cls: "badge-due", type: "pending" };
  if (status === "REJECTED")
    return { label: "Rejected", cls: "badge-overdue", type: "rejected" };
  ...
  if (status === "APPROVED" && dueDate) {
    ...
    if (diffDays < 0)
      return { label: "Overdue", cls: "badge-overdue", type: "overdue" };
    if (diffDays <= 5)
      return { label: "Due soon", cls: "badge-due", type: "due" };
...
async function renderUserHero() {
  ...
  document.getElementById("welcome_msg").textContent =
...
function renderMetrics(data) {
  document.getElementById("metric_borrowed").textContent =
    data.currently_borrowing ?? 0;
  ...
  document.getElementById("metric_total").textContent =
...
document.getElementById("dash_search_btn").addEventListener("click", dashSearch);
